...
- A removal claim is not success. Post-run, `src/verify.ts` reconciles both ways: claimed-but-present downgrades to failed; row-lingering-but-gone upgrades to removed. Classic playlists re-fetch the list server-side; the lockup list cannot be re-fetched and keeps removed cards on screen, so its report counts removals the page's own request completed and says so. Preserve this honesty in any new write path.
